...
LLM分野について、自分自身の知識およびスキルの向上を目指すとともに、この分野を学んでいる方が開発を始めるための一助となることを目指しています。
...
| **[Gemma 4 & Llama 4 ノートブック](https://github.com/yf591/llm-toolkit/blob/main/gemma-4_llama-4_notebook.ipynb)**                    | Google Gemma 4 や Meta Llama 4 などの最新LLMをGoogle Colab上で手軽に試せるノートブック。一問一答形式とチャット形式（会話履歴保持）の2つのモードをGradio UIで試せます。             | `transformers`, `torch`, `gradio`                                                                                              | 最新モデル対応 (Gemma 4 / Llama 4), 一問一答 & チャット形式, パラメータ調整, Gradio UIによる操作                                 | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/yf591/llm-toolkit/blob/main/gemma-4_llama-4_notebook.ipynb) |
...
### 4. Gemma 4 & Llama 4 ノートブック
Google Gemma 4 や Meta Llama 4 などの最新世代LLMをGoogle Colab上で手軽に試すためのノートブックです。`transformers` ライブラリを最新版にアップデートすることで新しいアーキテクチャに対応し、GradioベースのWeb UIで直感的に操作できます。

**2つのモード**
*   **一問一答形式**: システムプロンプトで役割を設定し、単発の回答を生成します。
*   **チャット形式（推奨）**: 過去の会話履歴を保持した連続した対話が可能です。`-it` や `-instruct` サフィックスのついたInstructチューニング済みモデルの使用を推奨します。

**こんな人におすすめ**
*   Gemma 4・Llama 4 など最新LLMを手軽に試してみたい。
*   一問一答とチャット形式の両方を比較したい。
*   Gradio UIを使ってパラメータを調整しながらテキスト生成を体験したい。
